...
main() {
	...
	TEST_URL="https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/README.md"
	RC=0
	...
	case "$OS" in
		ubuntu|debian|raspbian|centos|oracle|rhel|amazon-linux|opensuse|photon)
			...
			URL="https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/docs/$TRACK/$OS/$VERSION.md"
			$CURL "$URL" 2> /dev/null | grep "" || OS_UNSUPPORTED=1
			;;
		fedora)
	...
	if [ "$OS_UNSUPPORTED" = "1" ]; then
		case "$OS" in
			other-linux)
				...
				echo "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/#static"
				;;
			"")
	...
	case "$PACKAGETYPE" in
		apt)
			...
			case "$APT_KEY_TYPE" in
				legacy)
					$CURL "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION.asc" | $SUDO apt-key add -
					$CURL "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION.list" | $SUDO tee /etc/apt/sources.list.d/tailscale-mirror.list
					$SUDO chmod 0644 /etc/apt/sources.list.d/tailscale-mirror.list
				;;
				keyring)
					$CURL "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION.noarmor.gpg" | $SUDO tee /usr/share/keyrings/tailscale-mirror-archive-keyring.gpg >/dev/null
					$SUDO chmod 0644 /usr/share/keyrings/tailscale-mirror-archive-keyring.gpg
					$CURL "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION.tailscale-keyring.list" | $SUDO tee /etc/apt/sources.list.d/tailscale-mirror.list
					$SUDO chmod 0644 /etc/apt/sources.list.d/tailscale-mirror.list
				;;
			esac
			$SUDO apt-get update
		...
		yum)
			...
			$SUDO yum-config-manager -y --add-repo "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/tailscale.repo"
		...
		dnf)
			...
			if [ "$DNF_VERSION" = "3" ]; then
				...
				$SUDO dnf config-manager --add-repo "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/tailscale.repo"
			elif [ "$DNF_VERSION" = "5" ]; then
				# Already installed config-manager, above.
				$SUDO dnf config-manager addrepo --from-repofile="https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/tailscale.repo"
			else
		...
		tdnf)
			set -x
			curl -fsSL "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/tailscale.repo" > /etc/yum.repos.d/tailscale.repo
		...
		zypper)
			set -x
			$SUDO rpm --import "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/repo.gpg"
			$SUDO zypper --non-interactive ar -g -r "https://raw.githubusercontent.com/hopleus/tailscale-mirror/main/data/$TRACK/$OS/$VERSION/tailscale.repo"
